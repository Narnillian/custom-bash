...
# ~/.bash_aliases, instead of adding them the bash-doc package.

if [ -f ~/.bash_aliases ]; then
    . ~/.bash_aliases
fi

# enable programmable completion features (you don't need to enable, if it's already enabled in /etc/bash.bashrc and /etc/profile
...
if test "${homework,,}" = n || test "${homework,,}" = no; then
	...
	#	#if ringing, pass
	#	#else ring on own thread
	#	for i in {0..9}; do
	#		printf "\a"
	#		sleep 0.7
	#	done
	#else
	#	echo "change \$DING to true to hear dings"
	#fi
else
